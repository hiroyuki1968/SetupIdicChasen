========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : SetupIdicChasen
========================================================================


AppWizard が生成した SetupIdicChasen には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには SetupIdicChasen を構成している各ファイルの概要説明が含まれています。
...
SetupIdicChasen.h
    ...
    みます） をインクルードし、また CSetupIdicChasenApp アプリケーションクラスを宣
    言します。

SetupIdicChasen.cpp
    このファイルは CSetupIdicChasenApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\SetupIdicChasen.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル SetupIdicChasen.rc によってインクルード
    されます。

res\SetupIdicChasen.rc2
...
SetupIdicChasenDlg.h, SetupIdicChasenDlg.cpp - ダイアログ
    これらのファイルは CSetupIdicChasenDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な SetupIdicChasen.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) SetupIdicChasen.pch 
